document.addEventListener('DOMContentLoaded', () => {
    ...
    function captalizrFirstChar(str){
        if(!str) return str;

        return str.charAt(0).toUpperCase() + str.slice(1);
    } 

    function formDate(userFriendllyDate){
        const date = new Date(userFriendllyDate);
            ...
            tasks.forEach(task => {
                ...
                <div class="flex gap-3 items-center">
                    ...
                    <div class="flex flex-col">
                        <p class="font-bold text-lg leading-[27px]">${captalizrFirstChar(task.taskName)}</p>
                        <p class="text-sm leading-[21px] text-taskia-grey">Created at ${formDate(task.createdAt)}</p>
                    </div>
                </div>
